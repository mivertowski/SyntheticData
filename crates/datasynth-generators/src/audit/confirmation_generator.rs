...
impl ConfirmationGenerator {
	...
	pub fn generate_confirmations(
		...
		let count = self.rng.random_range(
			self.config.confirmations_per_engagement.0
				..=self.config.confirmations_per_engagement.1,
		) as usize;
		...
		for i in 0..count {
			...
			confirmation.confirmation_ref =
				format!("CONF-{}-{:04}", engagement.fiscal_year, self.confirmation_counter);
			...
			if roll < no_response_cutoff {
				...
				match response_type {
					...
					ResponseType::ConfirmedWithException => {
						...
						let exception_units =
							(balance_units as f64 * exception_pct).round() as i64;
...
mod tests {
	...
	fn test_generates_expected_count() {
		...
		let (confs, _) = gen.generate_confirmations(&engagement, &empty_workpapers(), &empty_accounts());

		let min = ConfirmationGeneratorConfig::default().confirmations_per_engagement.0 as usize;
		let max = ConfirmationGeneratorConfig::default().confirmations_per_engagement.1 as usize;
		assert!(
	...
	fn test_exception_amounts() {
		...
		let book_map: std::collections::HashMap<uuid::Uuid, Decimal> =
			confs.iter().map(|c| (c.confirmation_id, c.book_balance)).collect();

		let exceptions: Vec<&ConfirmationResponse> = responses
			.iter()
			.filter(|r| r.has_exception)
			.collect();

		assert!(!exceptions.is_empty(), "expected at least some exception responses");
