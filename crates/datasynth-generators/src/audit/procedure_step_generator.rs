...
use datasynth_core::models::audit::{
	AuditProcedureStep, Assertion, ProcedureType, StepProcedureType, StepResult, Workpaper,
};
...
impl ProcedureStepGenerator {
	...
	fn exception_text(&self, assertion: Assertion) -> &'static str {
		match assertion {
			...
			Assertion::Completeness => "Item exists in the population but was not recorded in the ledger.",
			Assertion::Accuracy => "Recorded amount differs from the supporting document by more than 1%.",
			Assertion::Cutoff => "Transaction recorded in the wrong accounting period.",
			Assertion::Classification => "Amount posted to incorrect expense or balance sheet account.",
			Assertion::Existence => "Asset could not be physically located or confirmed with a third party.",
			Assertion::RightsAndObligations => "Evidence of ownership or obligation could not be obtained.",
			Assertion::ValuationAndAllocation => "Carrying value is inconsistent with observable market inputs.",
			Assertion::PresentationAndDisclosure => "Disclosure is incomplete or does not meet the applicable framework.",
		}
	}
}
...
mod tests {
	...
	fn test_step_completion() {
		...
		let completed = steps.iter().filter(|s| s.status == StepStatus::Complete).count();
