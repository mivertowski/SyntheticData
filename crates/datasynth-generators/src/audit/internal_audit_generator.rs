...
impl InternalAuditGenerator {
	...
	pub fn generate(
		...
		let (objectivity, competence, assessment) = match reliance {
			RelianceExtent::NoReliance => {
				(ObjectivityRating::Low, CompetenceRating::Low, IaAssessment::Ineffective)
			}
			RelianceExtent::LimitedReliance => (
			...
			RelianceExtent::FullReliance => {
				(ObjectivityRating::High, CompetenceRating::High, IaAssessment::FullyEffective)
			}
		};
		...
		let quality_assurance =
			matches!(reliance, RelianceExtent::SignificantReliance | RelianceExtent::FullReliance);

		let head_name = self.head_of_ia_name();
		let qualifications = self.ia_qualifications(&reliance);

		let mut function = InternalAuditFunction::new(
		...
		for i in 0..report_count {
			...
			report.methodology = "Risk-based audit approach with control testing and data analytics."
				.to_string();
			...
			let rec_count = self.rng.random_range(
				self.config.recommendations_per_report.0
					..=self.config.recommendations_per_report.1,
			) as usize;
			...
			for _ in 0..rec_count {
				...
				let management_response =
					Some("Management accepts recommendation and will implement by target date.".to_string());
	...
	fn ia_qualifications(&mut self, reliance: &RelianceExtent) -> Vec<String> {
		...
		let mut remaining: Vec<&str> = all_quals.iter().copied().collect();
		for _ in 0..count {
