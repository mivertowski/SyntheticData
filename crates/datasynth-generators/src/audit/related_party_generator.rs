...
impl RelatedPartyGenerator {
	...
	pub fn generate(
		...
		let party_count = self
			.rng
			.random_range(self.config.parties_per_engagement.0..=self.config.parties_per_engagement.1)
			as usize;
	...
	fn generate_party(
		...
		if matches!(party_type, RelatedPartyType::Subsidiary | RelatedPartyType::Associate) {
	...
	fn generate_transactions(
		...
		let count = self
			.rng
			.random_range(self.config.transactions_per_party.0..=self.config.transactions_per_party.1)
			as usize;

		let mut txns = Vec::with_capacity(count);

		let fieldwork_days =
			(engagement.fieldwork_end - engagement.fieldwork_start).num_days().max(1);
...
mod tests {
	...
	fn sample_vendor_names() -> Vec<String> {
		vec!["Allied Components GmbH".to_string(), "BestSource Procurement".to_string()]
	}

	fn sample_customer_names() -> Vec<String> {
		vec!["Acme Industries Ltd".to_string(), "Beacon Holdings PLC".to_string()]
	}
	...
	fn test_generates_parties() {
		...
		let min = RelatedPartyGeneratorConfig::default().parties_per_engagement.0 as usize;
		let max = RelatedPartyGeneratorConfig::default().parties_per_engagement.1 as usize;
		assert!(
	...
	fn test_generates_transactions() {
		...
		let (parties, transactions) =
			gen.generate(&engagement, &sample_vendor_names(), &sample_customer_names());

		assert!(!transactions.is_empty(), "should generate at least one transaction");
