...
impl AnalyticalProcedureGenerator {
	...
	pub fn with_config(seed: u64, config: AnalyticalProcedureGeneratorConfig) -> Self {
		Self { rng: seeded_rng(seed, 0), config }
	}
	...
	pub fn generate_procedures(
		...
		phases.extend(std::iter::repeat_n(AnalyticalPhase::Planning, planning_count));
		phases.extend(std::iter::repeat_n(AnalyticalPhase::Substantive, substantive_count));
		phases.extend(std::iter::repeat_n(AnalyticalPhase::FinalReview, final_count));
		...
		for (i, &phase) in phases.iter().enumerate() {
			...
			let expectation_basis = format!(
				"Prior year adjusted for growth — {method:?} applied to {account_or_area}"
			);
			...
			if !matches!(conclusion, AnalyticalConclusion::Consistent) {
				result.explanation =
					Some(self.explanation_text(conclusion, &account_or_area));
				if matches!(conclusion, AnalyticalConclusion::ExplainedVariance) {
					result.explanation_corroborated = Some(true);
					result.corroboration_evidence =
						Some("Management provided supporting schedule; figures agreed to source data.".to_string());
				}
			}
	...
	fn explanation_text(
		&self,
		conclusion: AnalyticalConclusion,
		area: &str,
	) -> String {
		match conclusion {
			AnalyticalConclusion::ExplainedVariance => {
...
mod tests {
	...
	fn test_phase_distribution() {
		...
		let has_planning = results.iter().any(|r| r.procedure_phase == AnalyticalPhase::Planning);
		let has_substantive =
			results.iter().any(|r| r.procedure_phase == AnalyticalPhase::Substantive);
		let has_final =
			results.iter().any(|r| r.procedure_phase == AnalyticalPhase::FinalReview);

		assert!(has_planning, "expected at least one Planning procedure");
		assert!(has_substantive, "expected at least one Substantive procedure");
	...
	fn test_deterministic() {
		...
		assert_eq!(results_a.len(), results_b.len(), "lengths differ across identical seeds");
	...
	fn test_account_codes_used() {
		let engagement = create_test_engagement();
		let accounts = vec!["REV-1000".to_string(), "EXP-2000".to_string(), "ASS-3000".to_string()];
