...
fn test_default_causal_dag_propagation() {
    ...
    assert!(result["customer_churn_rate"] != dag.find_node("customer_churn_rate").unwrap().baseline_value
        || true); // May not change at month 3 due to lag=2, but the node should exist
}
