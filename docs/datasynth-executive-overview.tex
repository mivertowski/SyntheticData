...
\fancyhead[R]{\small\textcolor{midgray}{v2.0.0}}
...
\begin{flushleft}
  ...
  {\normalsize\textcolor{white!50}{Rust core \textbullet{} Python 1.10 \textbullet{} REST/gRPC API \textbullet{} Desktop UI \textbullet{} Audit FSM Engine}}
\end{flushleft}
...
\begin{flushleft}
  ...
  {\Large\textcolor{brand}{\textbf{Version 2.0.0}}}\\[1.5em]
  \textcolor{midgray}{%
...
\begin{flushleft}
  \textcolor{midgray}{\small
    Built with Rust \& Python \textbullet{} 19 modular crates \textbullet{} 200\,K+ entries/sec\\[4pt]
    Open-source (Apache-2.0) \textbullet{} Commercial license available\\[4pt]
    \textbf{GitHub:} \url{https://github.com/mivertowski/SyntheticData}
  }
\end{flushleft}
\end{titlepage}
...
\begin{itemize}[leftmargin=1.2em, itemsep=3pt]
  ...
  \item \textbf{Methodology-driven audit simulation} --- 12 audit methodology
        blueprints (ISA, IIA-GIAS, PCAOB, AICPA SOC~2, Big~4 firm-style)
        encoded as YAML-defined finite state machines, producing
        event-sourced audit trails with judgment-level classification
        and cross-firm benchmark comparison.
\end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[statbox, width=4.8cm]
  \centering
  {\fontsize{26}{30}\selectfont\bfseries\textcolor{accent}{12}}\\[3pt]
  {\small audit methodology blueprints}
\end{tcolorbox} &
\begin{tcolorbox}[statbox, width=4.8cm]
  \centering
  {\fontsize{26}{30}\selectfont\bfseries\textcolor{accent}{5}}\\[3pt]
  {\small audit frameworks (ISA/PCAOB/\ldots)}
\end{tcolorbox} &
\begin{tcolorbox}[statbox, width=4.8cm]
  \centering
  {\fontsize{26}{30}\selectfont\bfseries\textcolor{accent}{3}}\\[3pt]
  {\small judgment levels per step}
\end{tcolorbox}
\end{tabular}
\end{center}
...
Document Flows &
  ...
  three-way match validation, contract price enforcement, and credit memo
  generation (configurable \texttt{returns\_rate} with ARCreditMemo). \\
Intercompany &
...
Source-to-Pay &
  ...
  detection. \\
Hire-to-Retire &
  Payroll runs with progressive tax bracket computation (country-pack-aware),
  time entries with overtime modeling, expense reports with policy violation
  detection, and payroll journal entry generation. \\
Manufacturing &
...
\textit{New in v0.8.0, expanded in v0.11.0} --- pluggable JSON-based country configuration replacing
$\sim$7\,500 lines of hardcoded country-specific data:

\begin{itemize}[leftmargin=1.4em, itemsep=2pt]
  \item \textbf{10 built-in packs}: US, DE, GB, FR, JP, CN, IN, IT, ES, CA with
        16-section JSON schema covering locale, names, holidays, tax rates,
        addresses, phone formats, banking, payroll rules, legal entity formats,
        and document texts.
  ...
  \item \textbf{5 holiday types, 15 regions}: Fixed dates, Easter-relative,
        nth-weekday (with offset), last-weekday, and lunar calendar algorithms
        (Chinese New Year, Diwali, Eid al-Fitr, Chuseok). Holiday calendars
        for US, DE, GB, FR, IT, ES, CA, CN, JP, IN, BR, MX, AU, SG, KR.
...
\begin{itemize}[leftmargin=1.4em, itemsep=2pt]
  ...
  \item \textit{New in v0.11.0}: External confirmations per ISA 505 --- AR (40\%),
        AP (30\%), Bank (20\%), Legal (10\%) with positive/negative response
        modeling, reconciliation for disagreements, and alternative procedures
        for non-responses.
\end{itemize}

\subsection{YAML-Driven Audit FSM Engine}

\textit{New in v2.0.0} --- DataSynth introduces a methodology-driven approach to audit data
generation through the \texttt{datasynth-audit-fsm} crate. Rather than
generating audit artifacts through independent, stateless generators, the
engine loads audit methodology blueprints---encoded as YAML-defined finite
state machines---and walks each procedure's aggregate state machine in
topological order, respecting precondition dependencies.

\subsubsection{Two-Layer Architecture}

The engine separates \emph{what happens} (methodology blueprint) from
\emph{how it generates} (generation overlay):

\begin{itemize}[leftmargin=1.2em, itemsep=3pt]
  \item \textbf{Blueprints} define procedures, phases, state transitions,
        evidence requirements, actors, and standards references. Twelve
        built-in blueprints ship with v2.0.0, spanning ISA, IIA-GIAS,
        PCAOB, AICPA SOC~2, and Big~4 firm-style methodologies.
  \item \textbf{Overlays} control revision probability, timing distributions,
        artifact volumes, anomaly injection rates, and actor behavior profiles.
        Three presets are provided: \texttt{default}, \texttt{thorough}, and
        \texttt{rushed}.
  \item \textbf{Judgment levels} --- each step carries a
...
\begin{tabular}{@{}llrrr@{}}
\toprule
\textbf{Blueprint} & \textbf{Framework} & \textbf{Procedures} & \textbf{Steps} & \textbf{Standards} \\
\midrule
Generic FSA        & ISA        &  9 &  24 & 14 \\
Generic IA         & IIA-GIAS   & 34 &  82 & 52 \\
Deloitte / PwC / KPMG FSA & ISA & 10 &  28 & 16 \\
Deloitte / PwC / KPMG ISA (full) & ISA & 48 & 180 & 34 \\
...
\begin{itemize}[leftmargin=1.2em, itemsep=3pt]
  \item \textbf{Event-sourced audit trail} --- every state transition and
        procedure step emits a deterministic audit event with actor, timestamp,
        command, evidence references, and standards citations.
  \item \textbf{Concrete artifact generation} --- a step dispatcher maps 135
        blueprint commands to 14 pre-initialized generators, producing typed
        artifacts: materiality calculations, risk assessments, workpapers,
        evidence, sampling plans, analytical procedures, audit opinions, and more.
  \item \textbf{8-state C2CE lifecycle} --- the Condition--Criteria--Cause--Effect
        finding development workflow models realistic audit finding progression.
  \item \textbf{Self-loop handling} --- follow-up procedures with re-verification
        cycles are bounded by configurable iteration limits.
  \item \textbf{Continuous phases} --- governance, ethics, and quality phases
        execute in parallel with sequential audit phases.
  \item \textbf{Discriminator filtering} --- scope engagements by category
        (financial, operational, compliance, IT), risk rating, or engagement type.
  \item \textbf{OCEL 2.0 projection} --- audit events map to the Object-Centric
        Event Log standard for integration with process mining tools.
  \item \textbf{Streaming execution} --- callback-based API emits each
        audit event in real time during FSM traversal, enabling downstream
        consumption without buffering the full engagement result.
  \item \textbf{ISA 600 group audit} --- component auditor instructions,
        reports, and multi-entity coordination for group audit simulation.
  \item \textbf{Year-over-year engagement chains} --- multi-period audit
        simulation with prior-year comparative procedures and opening
        balance reliance.
\end{itemize}

\subsubsection{Audit Path Optimizer}

The companion \texttt{datasynth-audit-optimizer} crate converts methodology
blueprints into directed graphs for analytical queries:

\begin{itemize}[leftmargin=1.2em, itemsep=3pt]
  \item \textbf{Shortest path analysis} --- minimum transitions to completion
        (FSA: 27, IA: 101).
  \item \textbf{Constrained path} --- must-visit procedures with transitive
        precondition expansion.
  \item \textbf{Monte Carlo simulation} --- stochastic outcome distributions
        across $N$ engagement simulations, identifying bottleneck procedures,
        revision hotspots, and the statistically most common completion path.
\end{itemize}
...
\noindent
\textit{Major expansion in v0.10.0} --- a full \textbf{Counterfactual Simulation Engine}
for paired baseline/counterfactual dataset generation:

\begin{itemize}[leftmargin=1.4em, itemsep=2pt]
  \item \textbf{CausalDAG}: 16-node default financial process DAG with 8 transfer
        function types (Linear, Exponential, Logistic, Step, Threshold, Decay,
        Piecewise, InverseLogistic) and forward propagation with lag support.
  \item \textbf{8 InterventionTypes}: ParameterShift, MacroShock, ControlFailure,
        EntityEvent, ProcessChange, RegulatoryChange, Composite, and Custom ---
        each with timing, onset interpolation, and priority-based conflict resolution.
  \item \textbf{ConfigMutator}: Dot-path config mutation for applying causal
        propagation results to generation parameters.
  \item \textbf{DiffEngine}: Baseline vs. counterfactual comparison with summary,
        record-level, and aggregate diff formats including KPI impact analysis.
  \item \textbf{CLI}: \texttt{scenario \{list, validate, generate, diff\}} subcommands
        for end-to-end scenario management.
\end{itemize}

\subsection{Unified Generation Pipeline}

\textit{New in v0.11.0} --- stateful multi-period generation with fraud scenario
packs, phase-aware streaming, and enriched OCEL~2.0 event logs:

\begin{itemize}[leftmargin=1.4em, itemsep=2pt]
  \item \textbf{GenerationSession}: Stateful multi-period generation that splits
        output into fiscal-year-aligned periods, carrying forward balances, entity
        state, and document IDs between periods via \texttt{.dss} checkpoint files.
        Supports incremental generation (\texttt{--append --months~N}).
  \item \textbf{5 Fraud Scenario Packs}: Pre-built YAML configurations
        (\texttt{revenue\_fraud}, \texttt{payroll\_ghost}, \texttt{vendor\_kickback},
        \texttt{management\_override}, \texttt{comprehensive}) applied via deep-merge
        with \texttt{--fraud-scenario} and \texttt{--fraud-rate} CLI flags.
  \item \textbf{StreamPipeline}: Phase-aware streaming with file, HTTP, and no-op
        targets. JSONL envelope format with generation phase metadata for feeding
        downstream pipelines during generation.
  \item \textbf{OCEL 2.0 Enrichment}: Lifecycle state machines (PO, SO, VI with
        typed state transitions), multi-object correlation events (three-way match,
        payment allocation, bank reconciliation), and resource pool workload modeling
        with round-robin, least-busy, and skill-based assignment strategies.
\end{itemize}
...
\begin{itemize}[leftmargin=1.4em, itemsep=2pt]
  ...
  \item \textbf{Entity Relationship Graph}: 51+ entity types across 10 process families, 28+ relationship types with typed edge constraints, composite strength calculation from volume, count, duration, recency, and mutual connections.
...
\textit{Expanded in v0.11.0} --- comprehensive drift modeling with dedicated generators
for realistic temporal evolution:

\begin{itemize}[leftmargin=1.4em, itemsep=2pt]
  \item \textbf{Organizational Events}: \texttt{OrganizationalEventGenerator} produces Acquisition (1.35x volume, 5\% integration errors), Divestiture, Reorganization, LeadershipChange, WorkforceReduction, and Merger events with multi-phase integration timelines and volume/error-rate multipliers.
  \item \textbf{Process Evolution}: \texttt{ProcessEvolutionGenerator} with S-curve automation rollout, approval workflow transitions between \texttt{WorkflowType} variants, policy changes, and control enhancements.
  \item \textbf{Drift Event Meta-Generator}: \texttt{DriftEventGenerator} observes organizational and process events to produce ML ground-truth \texttt{LabeledDriftEvent} records with drift type (Statistical, Temporal, Behavioral, Market, Organizational, Process), magnitude, affected fields, and detection difficulty.
...
\begin{tikzpicture}[
  ...
  % Generator layer (row 2) — audit crates
...
Python Wrapper &
  \texttt{datasynth-py} 1.10 with blueprints (sourcing, HR, manufacturing,
...
\begin{tabularx}{\textwidth}{>{\bfseries\color{brand}}l X}
  ...
  Documentation &
    \url{https://mivertowski.github.io/SyntheticData/} \\
  Crates.io &
