...
[![Version](https://img.shields.io/badge/version-0.9.4-blue.svg)](https://github.com/ey-asu-rnd/SyntheticData)
...
**v0.9.4:** RustGraph Round 2 — `ToNodeProperties` trait mapping 51 entity types across 10 process families (Tax, Treasury, ESG, Project, S2C, H2R, MFG, GOV) to graph property maps with camelCase keys. Edge type registry with 28 new `RelationshipType` variants and typed `EdgeConstraint` validation. New models (BomComponent, InventoryMovement, BenefitEnrollment), denormalized name fields on transaction models, boolean query flags, and `GraphNode::from_entity()` bridge for graph export.
