...
\system{}: the impossibility of reliably recovering ground truth from
observed enterprise data, and how this motivates the forward-generation
approach.
...
\begin{equation}
\mathcal{A}(n, m) = n! \cdot \frac{1}{m!} \sum_{k=0}^{m} (-1)^{m-k}
  \binom{m}{k} k^n
...
$n$-element set to an $m$-element set is given by the inclusion-exclusion
formula $m! \cdot S(n, m)$ where $S(n,m)$ is the Stirling number of the
second kind~\citep{ivertowski2024hardware}.  Expanding:
\[
S(n, m) = \frac{1}{m!} \sum_{k=0}^{m} (-1)^{m-k} \binom{m}{k} k^n
\]
The total count of surjections is $n! \cdot S(n,m) / S(n,m) \cdot m! \cdot S(n,m) = n! / m! \cdot \sum \ldots$ which simplifies to \Cref{eq:stirling_matching}.
...
journal entries, exhaustive verification of the ground-truth accounting
network is computationally infeasible.  No algorithm can enumerate all
consistent configurations in polynomial time.
\end{corollary}
...
an independent probability $\varepsilon$ of detecting and correcting a
systematic error introduced at an earlier stage.  The probability that
the error reaches the final stage undetected is:
\begin{equation}
\Prob[\text{undetected}] = (1 - \varepsilon)^{k-1}
\label{eq:error_persistence}
\end{equation}
For typical detection rates $\varepsilon \in [0.01, 0.05]$ and $k = 6$:
\begin{center}
\begin{tabular}{ccc}
\toprule
$\varepsilon$ & $k$ & $\Prob[\text{undetected}]$ \\
\midrule
0.01 & 6 & 95.1\% \\
0.02 & 6 & 90.4\% \\
0.05 & 6 & 77.4\% \\
...
The error survives stage $i$ with probability $(1 - \varepsilon)$.
Since stages are independent, the probability of surviving all $k-1$
stages after introduction is $(1 - \varepsilon)^{k-1}$.
\end{proof}
...
known (Layer~3 completeness).

For decidability: since $D$ is finite and $\mathcal{G}$ is known, any
predicate $\phi$ can be evaluated by tracing the generative provenance
of the relevant records, which requires at most one pass through $D$.
\end{proof}
